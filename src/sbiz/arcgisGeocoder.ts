...
const ARCGIS_ADDRESS_ENDPOINT = 'https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates'

export function buildArcGisAddressUrl(query: string): URL {
...
export function parseArcGisAddressResponse(payload: unknown): GeocodePoint | null {
...
export async function geocodeAddressWithArcGis(query: string): Promise<GeocodePoint | null> {
  const response = await fetch(buildArcGisAddressUrl(query))

  if (!response.ok) {
    const body = await response.text().catch(() => '')
    throw new Error(`ArcGIS geocoding request failed: HTTP ${response.status}${body ? ` ${body}` : ''}`)
  }

  return parseArcGisAddressResponse(await response.json())
}
